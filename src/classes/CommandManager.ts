...
export class CommandManager extends EventEmitter {
  ...
  public async load(dirPath: string): Promise<void> {
    ...
    // const applicationCommands = await client.application?.commands.fetch().catch(console.log);
    ...
    for (const file of commandFiles) {
      ...
      if (fs.statSync(fileDir).isDirectory()) {
        const subcommandFiles = fs.readdirSync(fileDir);
        const group = new Collection<string, Command<unknown>>();

        for (const subcommandFile of subcommandFiles) {
          if (!subcommandFile.endsWith('.js') || subcommandFile.startsWith('__')) continue;
          
          const C: new () => Command<unknown> = require(path.join(fileDir, subcommandFile)).default;
      ...
      // 其他就是一般指令
        ...
        const C: new () => Command<unknown> = require(fileDir).default;
        const instance = new C();
