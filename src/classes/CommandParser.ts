...
export class CommandParser extends null {
  ...
  static ParseMessageOption: parseMessageOptionData = {
    ...
    async [ApplicationCommandOptionType.Boolean]({ data, preParsedArgs }) {
      ...
      const argument = preParsedArgs.shift() ?? null;
      if (!argument) {
        return { arg: argument, status: CommandParserOptionResultStatus[data.required ? "Required" : "Pass"] };
      }
    ...
    async [ApplicationCommandOptionType.Channel]({ data, preParsedArgs, message }) {
      let argument = preParsedArgs.shift() ?? null;
      if (!argument) {
        return { arg: argument, status: CommandParserOptionResultStatus[data.required ? "Required" : "Pass"] };
      }

      argument = MessageMentions.ChannelsPattern.test(argument) ? argument.slice(2, -1) : argument;
      const channel = await message.client.channels.fetch(argument).catch(() => {}) ?? null;
      return { arg: argument, status: CommandParserOptionResultStatus[!channel ? "WrongFormat" : "Pass"] };
    },
    ...
    async [ApplicationCommandOptionType.Integer]({ data, preParsedArgs }) {
      ...
      if ('choices' in data && data.choices) {
        ...
        if (argument === null) {
          return { arg: null, status: CommandParserOptionResultStatus.NotInChoices, choices: data.choices };
        }
      }
      ...
      if ('minValue' in data) {
        if (data.minValue && number < data.minValue) {
          return { arg: null, status: CommandParserOptionResultStatus.ValueTooSmall, limit: data.minValue };
        }
        if (data.maxValue && data.maxValue < number) {
          return { arg: null, status: CommandParserOptionResultStatus.ValueTooLarge, limit: data.maxValue };
        }
      }
    ...
    async [ApplicationCommandOptionType.Mentionable]({ data, preParsedArgs, message }) {
      ...
      if (MessageMentions.RolesPattern.test(argument)) {
        const role = await message.guild?.roles.fetch(argument.slice(3, -1)).catch(() => {}) ?? null;
        return { arg: role, status: CommandParserOptionResultStatus[!role ? "WrongFormat" : "Pass"] };
      }
      if (MessageMentions.UsersPattern.test(argument)) {
        const user = await message.client.users.fetch(argument.slice(2, -1)).catch(() => {}) ?? null;
        return { arg: user, status: CommandParserOptionResultStatus[!user ? "WrongFormat" : "Pass"] };
      }
      const mentionable = 
        (await message.guild?.roles.fetch(argument).catch(() => {})) ?? 
        (await message.client.users.fetch(argument).catch(() => {})) ?? null;
      return { arg: mentionable, status: CommandParserOptionResultStatus[!mentionable ? "WrongFormat" : "Pass"] };
    },
    ...
    async [ApplicationCommandOptionType.Role]({ data, preParsedArgs, message }) {
      ...
      const role = await message.guild?.roles.fetch(argument).catch(() => {}) ?? null;
      return { arg: role, status: CommandParserOptionResultStatus[!role ? "WrongFormat" : "Pass"] };
    },
    ...
    async [ApplicationCommandOptionType.String]({ data, preParsedArgs }) {
      ...
      if ('minLength' in data) {
        if (data.minLength && argument.length < data.minLength) {
          return { arg: null, status: CommandParserOptionResultStatus.LengthTooShort, limit: data.minLength };
        }
        if (data.maxLength && data.maxLength < argument.length) {
          return { arg: null, status: CommandParserOptionResultStatus.LengthTooLong, limit: data.maxLength };
        }
      }
    ...
    async [ApplicationCommandOptionType.User]({ data, preParsedArgs, message }) {
      ...
      if (data.parseAs === CommandOptionType.Member) {
        const member = await message.guild?.members.fetch(argument).catch(() => {}) ?? null;
        return { arg: member, status: CommandParserOptionResultStatus[!member ? "WrongFormat" : "Pass"] };
      }
      const user = await message.client.users.fetch(argument).catch(() => {}) ?? null;
      return { arg: user, status: CommandParserOptionResultStatus[!user ? "WrongFormat" : "Pass"] };
    }, 
